 ...
 */
@Repository
public class GoodtypeDAO extends BaseHibernateDAO {
	...
	public Goodtype findById(java.lang.Integer id) {
		...
		try {
			Goodtype instance = (Goodtype) getSession().get("domain.Goodtype", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Goodtype instance) {
		...
		try {
			List results = getSession().createCriteria("domain.Goodtype")
	...
	public Goodtype merge(Goodtype detachedInstance) {
		...
		try {
			Goodtype result = (Goodtype) getSession().merge(detachedInstance);
